...
class xs_lotonuoi extends Client {

    function __construct() {
        parent::__construct();
        $this->load->model('xs_lotonuoi_model');
    }

    function detail($title_link = '') {
        $row_news = $this->xs_lotonuoi_model->get_by(array('title_link' => $title_link, 'active' => 'yes'));

        if ($row_news) {
            $data = array('view' => $row_news->view + 1);
            $this->xs_lotonuoi_model->update($row_news->id, $data);

            $row_news->title = trim($row_news->title);
            $row_news->content = trim($row_news->content);
            $this->data['row_news'] = $row_news;

            $this->data['related_news'] = $this->xs_lotonuoi_model->order_by('created_date', 'DESC')->limit(10)->get_many_by(array('active' => 'yes', 'id <>' => $row_news->id));
        ...
        $this->data['tmpl'] = 'xs_lotonuoi/detail';
