...
//        $nextday_loto = $date_chot;
		$this->data['_meta']['title'] = "Thảo luận, chốt số loto";
		$this->data['_meta']['description'] = "Thảo luận, chốt số loto";
		$this->data['_meta']['keywords'] = "chot so loto, chốt số loto";
...
//        }
//        $this->db->distinct();
//        $topdaigia = $this->db->select("a.userid,b.fullname,b.email")->from("xs_loto_online AS a")
...
//                $topdaigia[$i]['taikhoan'] = $this->db->select("taikhoan")->from("xs_loto_online")
...
//
//            $sort_tk = array();
//            foreach ($topdaigia as $key => $value) {
//                $sort_tk[$key] = $value['taikhoan'];
//            }
//            array_multisort($sort_tk, SORT_DESC, $topdaigia);
//        }
        ...
        $arrtmp_caplon = array();
        $dem_cap = 0;
        foreach ($this->data['itemsImportant']['cautious'] as $k => $v) {
            $arr = str_split($v['number']);
            if ($arr[0] != $arr[1]) {
                foreach ($this->data['itemsImportant']['cautious'] as $item) {
                    if (!in_array($arr[1] . $arr[0], $arrtmp_caplon) && $arr[1] . $arr[0] == $item['number']) {
                        $arrtmp_caplon[] = $v['number'];
                        $arr_caplon[$dem_cap]->number = $v['number'];
                        $arr_caplon[$dem_cap]->not_count = $v['not_count'];
                        $arr_caplon[$dem_cap]->lon = $item['number'];
                        $arr_caplon[$dem_cap]->lon_not_count = $item['not_count'];
                        $dem_cap++;
                    }
                }
            }
            ...
            if ($v['not_count'] >= 15) {
                $where = ' AND CONCAT_WS(\',\',b0,b1,b2,b3,b4,b5,b6,b7,b9) LIKE \'%' . $v['number'] . ',%\'';
                $query = 'SELECT `date`
                    FROM `xs_result`
                    WHERE `lid`=1'
                        . $where
...
//                            $date_end = $data[0]->date;
                        ...
                        if (isset($data[$i + 1])) {
                            ...
                            if ($khoangcach < 40 && $khoangcach > $max_khoangcach) {
                                $max_khoangcach = $khoangcach;
                                $arr_gancucdai[$v['number']]->khoangcach = $max_khoangcach;
                                $arr_gancucdai[$v['number']]->from_date = date('Y-m-d', strtotime($data[$i + 1]->date . ' +1 days'));
                                $arr_gancucdai[$v['number']]->to_date = date('Y-m-d', strtotime($item->date . ' -1 days'));
                            }
                        }
                    }
                }
        ...
        $this->data["arr_caplon"] = $arr_caplon;
        $this->data["arr_gancucdai"] = $arr_gancucdai;
        ...
        $data = $this->db->select('r.*, l.id AS lid, l.name, l.code, l.alias, l.area')
                ...
                ->get()
                ->row();
        $data->dateOfWeek = $this->xs_result_model->getDateOfWeek($data->date);
        $data->date = date('d/m/Y', strtotime($data->date));
        $this->data['items'] = $data;

//        $this->data["topdaigia"] = $topdaigia;
...
//        $this->data["tong_nguoichoi"] = $tong_nguoichoi;
        $this->data["tmpl"] = "chat/index";
        $this->load->view("layout/chat", $this->data);
    }

    function lototop() {
        $tuan = (isset($_GET['tuan']) ? $_GET['tuan'] : '');
        $thang = (isset($_GET['thang']) ? $_GET['thang'] : '');
        if ($tuan) {
            $loto_top_tuan = $this->db->select("a.userid,a.taikhoan,b.fullname")
...
//                    ->where("a.taikhoan >", 0)
            ...
            $loto_top_thang = $this->db->select("a.userid,a.taikhoan,b.fullname")
                    ->from("xs_loto_top AS a")
                    ->join("users AS b", "a.userid=b.id")
                    ->where("a.thang", $thang)
//                    ->where("a.taikhoan >", 0)
...
//        echo $this->db->last_query();
	...
	public function chatm2() {
        $this->load->view("chat/chatm2", $this->data);
    }
...
//                echo $this->db->last_query();
 ...
 // black key
                $arr_msg = explode(' ', $msg);
                $black_key = $this->db->select("*")
                ->from("xs_loto_keyword")
                ->where('id = 1')
                ->get()->row();
                //                 echo $this->db->last_query();
                for ($i=0; $i<count($arr_msg); $i++){
                
                	if (strpos($black_key->key_vn, $arr_msg[$i]) !== false) {
                		$msg = 'black keyword';
                		echo $msg;
                		die;
                	}
