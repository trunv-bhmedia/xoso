...
$banner_f = '';
foreach ($banner as $v) {
    if ($banner_f == '' && $v->position == 'bottom_right' && ($v->page == 'all' || $v->page == $c_module)) {
        $banner_f = '<a target="_blank" href="' . $v->url . '" title="' . view_title($v->name) . '"><img style="width: 260px;" src="' . site_url($v->image) . '" alt="' . view_title($v->name) . '" /></a>';
    }
}
if ($banner_f != '') {
    ?>
    ...
    <script type="text/javascript">
        ...
        var right_float_minHeight = 16;
        var right_float_curHeight = 0;
        function right_float_show( ){
            right_float_curHeight = right_float_maxHeight;
        ...
        //window.setTimeout("right_float_clickhide()", 8000);
