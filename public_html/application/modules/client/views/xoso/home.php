<?php $arr_banner_middle=array();foreach($banner as $v){if($v->position=='middle'&&($v->page=='all'||$v->page==$c_module)){$arr_banner_middle[]='<div><a target="_blank" href="'.$v->url.'" title="'.view_title($v->name).'"><img src="'.site_url($v->image).'" width="566" alt="'.view_title($v->name).'" /></a></div>';}}?>
...
//}
?>
<?php if(isset($date)&&$date!='')echo '<div class="title-top"><h1>KẾT QUẢ XỔ SỐ NGÀY '.str_replace('-','/',$date).'</h1><div class="tabs-note clearfix"><a class="span-tttt" href="'.$uri_root.'tuong-thuat-truc-tiep-ket-qua-xo-so.html">Tường thuật trực tiếp >></a><a class="span-tkxs" href="'.$uri_root.'thong-ke-quan-trong.html">Thống kê xổ số</a></div></div>';else echo '<div class="title-top"><h1>KẾT QUẢ XỔ SỐ</h1><div class="tabs-note clearfix"><a class="span-tttt" href="'.$uri_root.'tuong-thuat-truc-tiep-ket-qua-xo-so.html">Tường thuật trực tiếp >></a><a class="span-tkxs" href="'.$uri_root.'thong-ke-quan-trong.html">Thống kê xổ số</a></div></div>';$last_id=0;foreach($items as $k=>$v){$last_id=$v->id;$date_ve_do=str_replace('/','-',$v->date);
//if ((!isset($_SESSION['user']) || $_SESSION['user']['gender'] == 0) && date('Y-m-d',  strtotime($date_ve_do)) < '2008-01-01') break;
$v->extra=json_decode($v->extension);$title_share='Xổ Số '.$v->name.' - '.$v->dateOfWeek.' ngày '.$v->date;$shared_content='Xổ Số '.$v->name.' ngày '.$v->date.',';$shared_content.=' giải DB: '.$v->a0.',';$shared_content.=' giải nhất: '.$v->a1.',';$shared_content.=' giải nhì: '.$v->a2.',';$shared_content.=' giải ba: '.$v->a3.',';$shared_content.=' giải tư: '.$v->a4.',';$shared_content.=' giải năm: '.$v->a5.',';$shared_content.=' giải sáu: '.$v->a6.',';$shared_content.=' giải bảy: '.$v->a7;if($v->area!='MB')$shared_content.=', giải tám: '.$v->a8;$alias_date=$v->alias.'/'.str_replace('/','-',$v->date);$curPageURL=urlencode($uri_root.$alias_date.'.html');$url_google='https://www.google.com/bookmarks/mark?op=add&amp;bkmk='.$curPageURL.'&amp;title='.urlencode($title_share).'&amp;annotation='.$shared_content;$url_facebook='http://www.facebook.com/sharer.php?u='.$curPageURL;$url_yahoo='http://www.addtoany.com/add_to/yahoo_mail?linkurl='.$curPageURL.'. '.$shared_content.'&amp;type=page&amp;linkname='.$title_share.'&amp;linknote=';$url_email='mailto:?subject='.formatMail($title_share).'&amp;body='.$curPageURL.'. '.formatMail($shared_content);if($k==1){?>
...
<br/>
<?php }if($v->area=='MB'){$v->data[0]=$v->a0;$v->data[1]=$v->a1;$v->data[2]=$v->a2;$v->data[3]=$v->a3;$v->data[4]=$v->a4;$v->data[5]=$v->a5;$v->data[6]=$v->a6;$v->data[7]=$v->a7;?>
...
<table class="tbl-tt tbl-main-tt">
    <tr>
        <td colspan="2" class="bg-yelow1">
            Kí hiệu trúng giải đặc biệt
        </td>
        <td colspan="2" class="bg-yelow1">
            <?php echo $v->khtt; ?>
        </td>
...
<?php $str=str_replace('-','</strong><strong class="span-space">',$v->data[2]);echo '<strong class="span-space">'.$str.'</strong>';?>
</td>
</tr>
<tr>
<td class="border-right">Giải ba</td>
<td class="border-right giai3">
<?php $str=str_replace('-','</strong><strong class="span-space">',$v->data[3]);echo '<strong class="span-space">'.$str.'</strong>';?>
</td>
</tr>
<tr>
<td class="bg-gray border-right">Giải tư</td>
<td class="bg-gray border-right giai4">
<?php $str=str_replace('-','</strong><strong class="span-space">',$v->data[4]);echo '<strong class="span-space">'.$str.'</strong>';?>
</td>
</tr>
<tr>
<td class="border-right">Giải năm</td>
<td class="border-right giai5">
<?php $str=str_replace('-','</strong><strong class="span-space">',$v->data[5]);echo '<strong class="span-space">'.$str.'</strong>';?>
</td>
</tr>
<tr>
<td class="bg-gray border-right">Giải sáu</td>
<td class="bg-gray border-right giai6">
<?php $str=str_replace('-','</strong><strong class="span-space">',$v->data[6]);echo '<strong class="span-space">'.$str.'</strong>';?>
</td>
</tr>
<tr>
<td class="border-right">Giải bảy</td>
<td class="border-right giai7">
<?php $str=str_replace('-','</strong><strong class="span-space">',$v->data[7]);echo '<strong class="span-space">'.$str.'</strong>';?>
</td>
<td></td>
</tr>
</table>
...
<li>Để nhận kết quả xổ số <strong>Miền Bắc</strong> sớm nhất, soạn tin <span>BHX KQ MB</span> gửi <span>8588</span></li>
</ul>
...
<?php }else{if($v->area=='MT'){$l_area='MIỀN TRUNG';$url=$url_mientrung;$lid=2;}else{$l_area='MIỀN NAM';$url=$url_miennam;$lid=3;}?>
<div class="title title-red">
...
<td class="bg-gray border-right">Giải đặc biệt</td>
<td class="bg-gray border-right giaidb">
<?php echo '<strong class="red font18 span-space">'.$v->a0.'</strong>';?>
</td>
</tr>
<tr>
<td class="border-right">Giải nhất</td>
<td class="border-right giai1">
<?php echo '<strong class="span-space">'.$v->a1.'</strong>';?>
</td>
</tr>
<tr>
<td class="bg-gray border-right">Giải nhì</td>
<td class="bg-gray border-right giai2">
<?php $str=str_replace(array('-'),array('</strong><strong class="span-space">'),$v->a2);echo '<strong class="span-space">'.$str.'</strong>';?>
</td>
</tr>
<tr>
<td class="border-right">Giải ba</td>
<td class="border-right giai3">
<?php $str=str_replace(array('-'),array('</strong><strong class="span-space">'),$v->a3);echo '<strong class="span-space">'.$str.'</strong>';?>
</td>
</tr>
<tr>
<td class="bg-gray border-right">Giải tư</td>
<td class="bg-gray border-right giai4">
<?php $str=str_replace(array('-'),array('</strong><strong class="span-space">'),$v->a4);echo '<strong class="span-space">'.$str.'</strong>';?>
</td>
</tr>
<tr>
<td class="border-right">Giải năm</td>
<td class="border-right giai5">
<?php $str=str_replace(array('-'),array('</strong><strong class="span-space">'),$v->a5);echo '<strong class="span-space">'.$str.'</strong>';?>
</td>
</tr>
<tr>
<td class="bg-gray border-right">Giải sáu</td>
<td class="bg-gray border-right giai6">
<?php $str=str_replace(array('-'),array('</strong><strong class="span-space">'),$v->a6);echo '<strong class="span-space">'.$str.'</strong>';?>
</td>
</tr>
<tr>
<td class="border-right">Giải bảy</td>
<td class="border-right giai7">
<?php $str=str_replace(array('-'),array('</strong><strong class="span-space">'),$v->a7);echo '<strong class="span-space">'.$str.'</strong>';?>
</td>
</tr>
<tr>
<td class="bg-gray border-right">Giải tám</td>
<td class="bg-gray border-right giai8">
<?php echo '<strong class="span-space">'.$v->a8.'</strong>';?>
</td>
<td class="bg-gray"></td>
</tr>
</table>
...
<div class="line-red">&nbsp;</div>
<ul class="list-editor space1">
<li>Để nhận kết quả xổ số <strong><?php echo $v->name?></strong> sớm nhất, soạn tin <span>BHX KQ <?php echo $v->code?></span> gửi <span>8188</span></li>
</ul>
<div class="tabs-note col3 clearfix">
<a class="span-in" target="_blank" href="<?php echo $uri_root?>in-ve-do.html?l=<?php echo $lid?>&amp;d=<?php echo $date_ve_do?>&amp;t=2">&nbsp;</a>    
<a class="span-dvo" href="<?php echo $uri_root?>do-ve-so.html">&nbsp;</a>
...
<input type="hidden" id="kqxs_date_page" name="kqxs_date_page" value="<?php echo $last_id ?>" />
