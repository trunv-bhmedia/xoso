...
<div id="divAdRight" style="display: none; position: fixed; top: 32px;">
<?php 
//var_dump($banner);
foreach($banner as $v){
	if($v->position=='scrollleft' && ($v->page == 'all' || $v->page == $c_module)){?>
<div><a target="_blank" href="<?php echo $v->url;?>" title="<?php echo view_title($v->name);?>"><img src="<?php echo site_url($v->image);?>" width="<?php echo $v->width;?>" alt="<?php echo view_title($v->name);?>" /></a></div>
<?php }}?>
</div>
<div id="divAdLeft" style="display: none; position: fixed; top: 32px;">
<?php foreach($banner as $v){
	if($v->position=='scrollright' && ($v->page == 'all' || $v->page == $c_module)){?>
<div><a target="_blank" href="<?php echo $v->url;?>" title="<?php echo view_title($v->name);?>"><img src="<?php echo site_url($v->image);?>" width="<?php echo $v->width;?>" alt="<?php echo view_title($v->name);?>" /></a></div>
<?php }}?>
</div>
    ...
    <div class="mainmenu-wrapp">
        <div class="mainmenu clearfix">
            ...
            <div class="main-menu" id="smoothmenu">
                <ul id="nav">
                    <li class="first nav-home"><a href="<?php echo $uri_root ?>" title="xổ số"><span>&nbsp;</span></a></li>
...
<?php foreach ($news_category as $value) {
    echo '<li><a href="' . $uri_root . 'tin-xo-so/danh-muc-' . $value->name_link . '.html"><span>' . $value->name . '</span></a></li>';
					...
					<li><a href="/xo-so-dien-toan.html"><span>Điện toán</span></a>
						<ul class="submenu-border sub-menu">
							<div class="top-menu-popup">
								<li><a href="https://xoso.com/vietlott/xo-so-power-6-55-vietlott.html"><span>Kết quả Power 6/55</span></a></li>
	...
	<!--div  style='width:970px;text-align:center' class="mainmenu">
		<a href="http://www.tivi988.com/vi-vn/login/logon/212860"  target="_blank"><img src="http://cdn.smail188.com/banners/new_design/GIF/970x100.gif"></a>
	</div-->
</div>