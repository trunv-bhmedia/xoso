...
class toploto extends Admin {

    function __construct() {
        parent::__construct();
        $this->load->model(array('xs_loto_top_model', 'user_model'));
    }
...
//                    echo $this->db->last_query();
      ...
      //  $top_list = $this->db->select("*")
		$top_list = $this->db->select("a.*,b.fullname")
                        ->from('xs_loto_top as a')
						->join("users AS b", "a.userid=b.id")
						// ->where("a.ngay > '2015-01-27'")
                        ->order_by('a.taikhoan', 'DESC')
                        ->get()->result();

        $this->data['top_list'] = $top_list;

        $this->data["nextday"] = $nextday;
        $this->data["date"] = $date;
        $this->data['tpl_file'] = 'toploto/index';
