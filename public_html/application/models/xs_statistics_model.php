...
//                echo $v_list->str.'|'.$str_len.'<br/><br/>';
    ...
    function getItemsDetectDB($amplitude, $lid) {
        $area = 'MB';
        foreach ($this->data['xs_location_menu'] as $value) {
            if ($lid == $value->id) {
                $area = $value->area;
        ...
        if ($area == 'MB') {
            $query = "SELECT r.date,REPLACE(REPLACE(CONCAT_WS(',',a0,a1,a2,a3,a4,a5,a6,a7),'-',''),',','') AS str,b0 AS data
                    FROM xs_result as r
                    WHERE r.lid=" . Quote($lid)
                    . " ORDER BY r.date DESC"
                    . " LIMIT 0," . $amplitude
            ...
            $query = "SELECT r.date,REPLACE(REPLACE(CONCAT_WS(',',a8,a7,a6,a5,a4,a3,a2,a1,a0),'-',''),',','') AS str,b0 AS data
                    FROM xs_result as r
                    WHERE r.lid=" . Quote($lid)
                    . " ORDER BY r.date DESC"
                    . " LIMIT 0," . $amplitude
...
//                echo $v_list->str.'|'.$str_len.'<br/><br/>';
    ...
    function getItemsDetectOnlyDB($amplitude, $lid) {
        $query = "SELECT r.date,REPLACE(REPLACE(CONCAT_WS(',',a0,a1,a2,a3,a4,a5,a6,a7),'-',''),',','') AS str,b0 AS data
                    FROM xs_result as r
                    WHERE r.lid=" . Quote($lid)
                . " ORDER BY r.date DESC"
    ...
    function soiCauNew($lid, $ngay, $db, $lon, $nhay, $limit) {
        $select = "CONCAT_WS(',',b0,b1,b2,b3,b4,b5,b6,b7)";
        if ($db == 1)
            $select = "b0";
        $query = "SELECT r.date,REPLACE(REPLACE(CONCAT_WS(',',a0,a1,a2,a3,a4,a5,a6,a7),'-',''),',','') AS str," . $select . " AS data
                    FROM xs_result as r
                    WHERE r.lid=" . Quote($lid)
                . " AND r.date <" . Quote(date('Y-m-d', strtotime($ngay)))
                . " ORDER BY r.date DESC"
                . " LIMIT 0,16"
        ;
        $list = $this->db->query($query)->result();
//        echo $this->db->last_query();
        ...
        $max_cau = 1;
        $data_limit = array();
        $data_nextlimit = array();
        foreach ($list as $i => $item) {
            if ($i == 0 && !isset($_GET['ngay']))
                $ngay = date('d-m-Y', strtotime($item->date . ' +1 days'));

            $arr_data = explode(',', $item->data);
            if (isset($list[$i + 1])) {
                $str = $list[$i + 1]->str;
                foreach ($arr_data as $value) {
                    $arr = str_split($value);
                    $pos0 = strpos($str, $arr[0]);
                    if ($pos0 !== false) {
                        $pos1 = strpos($str, $arr[1]);
                        if ($pos1 !== false) {
                            $dem = 0;
                            foreach ($arr_data as $v) {
                                if ($lon == 1) {
                                    if ($v == $value || $v == $arr[1] . $arr[0])
                                        $dem++;
                                }else {
                                    if ($v == $value)
                                        $dem++;
                            ...
                            if ($pos0 != $pos1) {
                                $so_cua_cau = substr($list[$i]->str, $pos0, 1) . substr($list[$i]->str, $pos1, 1);

                                $vitri = $pos0 . 'x' . $pos1;
                                $vitri_dao = $pos1 . 'x' . $pos0;
                                if ($pos0 > $pos1 && $lon == 1) {
                                    $vitri = $pos1 . 'x' . $pos0;
                                    $vitri_dao = $pos0 . 'x' . $pos1;
                                    $so_cua_cau = substr($list[$i]->str, $pos1, 1) . substr($list[$i]->str, $pos0, 1);
                                }

                                if (isset($data[$vitri]['cau']) && $data[$vitri]['cau'] == $i) {
                                    $data[$vitri]['cau'] = $i + 1;
                                    if ($data[$vitri]['cau'] > $max_cau)
                                        $max_cau = $data[$vitri]['cau'];
                                } elseif ($i == 0) {
                                    if (!isset($data[$vitri_dao]['cau']) && $lon == 1) {
                                        $data[$vitri]['cau'] = 1;
                                        $data[$vitri]['so'] = $so_cua_cau;
                                    } elseif ($lon == 0) {
                                        $data[$vitri]['cau'] = 1;
                                        $data[$vitri]['so'] = $so_cua_cau;
                                    }
                                ...
                                if ($data[$vitri]['cau'] == $limit)
                                    $data_limit[$vitri] = $data[$vitri]['so'];
                                elseif ($data[$vitri]['cau'] == $limit + 1)
                                    $data_nextlimit[$vitri] = $data[$vitri]['so'];
                            }

                            $pos1 = strpos($str, $arr[1], $pos1 + 1);
                            while ($pos0 !== false && $pos1 !== false) {
                                if ($pos0 != $pos1) {
                                    $so_cua_cau = substr($list[$i]->str, $pos0, 1) . substr($list[$i]->str, $pos1, 1);
        ...
        $arr_rs = array();
        $arr_rs['matrancau'] = $list[0];
        $arr_rs['ngay'] = $ngay;
        $arr_rs['max_cau'] = $max_cau;
        $arr_rs['data'] = $data;
        $arr_rs['data_limit'] = $data_limit;
        $arr_rs['data_nextlimit'] = $data_nextlimit;
