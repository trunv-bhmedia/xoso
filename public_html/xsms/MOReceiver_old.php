...
//        file_get_contents('http://data.xoso.com/live.php?898=1'); //213
...
//    }
    ...
    if ($InContent != '' && strtoupper($InContent) == 'DV') {
        if ($SendFrom == _8017 || $SendFrom == _8717) {
...
//                $procresult = '0'; //test
        ...
        if (preg_match('/(KQ|TT)\s+.*/ism', $InContent, $m)) {
            $item_id = '';
            $time = 0;
            $so = '';

//        if (preg_match('/TT TK\s+([\w]+)\s+([\d]+)/ism', $InContent, $matchs)) {
//            $so = $matchs[2];
//            if (trim($so) != '') {
...
//            }
//        }
            if ($InContent != '' && strtoupper($InContent) == 'KQ MM') {
                $item_id = 'KQ MM_MB';
...
function SendMT($dienThoai, $sendfrom, $keyword, $outcontent, $flag, $SeqNo, $Total) {
    ...
    if ($err) {
//        $str = '<h2>Constructor error</h2><pre>' . $err . '</pre>' . "\n";
        exit;
    } else {
//        $str = '<h2>Successfully connected</h2>' . "\n";
    }
...
function updateXOSO($mobile, $item_id, $dau_so_nhan_tin, $msgbody, $time, $so) {
    ...
    $day30 = 0;
    if ($dau_so_nhan_tin == _8717 && $KeywordName == 'KQ')
        $day30 = 10;
    elseif ($dau_so_nhan_tin == _8517 && $KeywordName == 'KQ')
        $day30 = 5;

    if ($day30 > 0) {
        $query = 'SELECT id FROM xs_sms WHERE mobile=\'' . $mobile . '\' AND item_id=\'' . $item_id . '\' AND (dau_so_nhan_tin=\'' . _8717 . '\' OR dau_so_nhan_tin=\'' . _8517 . '\')';
        $row = $_db->setQuery($query);
...
//        $rs->status = 1;
        ...
        return $rs;
    } elseif ($KeywordName == 'TT' && $dau_so_nhan_tin == _8517) {
        $query = 'SELECT id FROM xs_sms WHERE mobile=\'' . $mobile . '\' AND item_id=\'' . $item_id . '\' AND day30=999 AND dau_so_nhan_tin=\'' . _8517 . '\'';
        $row = $_db->setQuery($query);
        $row = $_db->loadObject($item);
    ...
    } elseif ($KeywordName == 'KQ MM' && ($dau_so_nhan_tin == _8317 || $dau_so_nhan_tin == _8517)) {
        ...
        $query = 'SELECT id,a_all FROM xs_sms WHERE 
                    mobile=\'' . $mobile . '\'
                    AND item_id=\'' . $item_id . '\'
                    AND (dau_so_nhan_tin=\'' . _8317 . '\' OR dau_so_nhan_tin=\'' . _8517 . '\')
    ...
    if ($KeywordName == 'KQ' && $dau_so_nhan_tin == _8017) {
        $rs = KQXS($xs_location_code, $xs_location->name, $time);
        return $rs;
    } elseif ($KeywordName == 'TT TK' && $dau_so_nhan_tin == _8517) {
