...
function get_cat() {
    $db = new MyDBO();
    $sql = "SELECT * FROM `vietlott_crawl` WHERE id = 6 LIMIT 1";
...
//    var_dump($obj_content); die;

    if (!$obj_content->content) {
        return false;
    }
    $web = loadHtmlString($obj_content->content);
    $giai_dac_biet_obj = $web->find('ul[class=result-number-mega655]', 0);
    ...
    $array_db[] = trim(strip_tags($list_number_db[8]->innertext));
...
//    echo($detail);die;
    ...
    $array_dt['jp2']['sl'] = trim(strip_tags($list_dt[2]->find('td', 2)->innertext));
    $array_dt['jp2']['gt'] = trim(strip_tags($list_dt[2]->find('td', 3)->innertext));

    $array_dt['g1']['sl'] = trim(strip_tags($list_dt[3]->find('td', 2)->innertext));
    $array_dt['g1']['gt'] = trim(strip_tags($list_dt[3]->find('td', 3)->innertext));

    $array_dt['g2']['sl'] = trim(strip_tags($list_dt[4]->find('td', 2)->innertext));
    $array_dt['g2']['gt'] = trim(strip_tags($list_dt[4]->find('td', 3)->innertext));

    $array_dt['g3']['sl'] = trim(strip_tags($list_dt[5]->find('td', 2)->innertext));
    $array_dt['g3']['gt'] = trim(strip_tags($list_dt[5]->find('td', 3)->innertext));

    if (!$array_dt['g3']['sl']) {
        return false;
    }
    $str_next = $list_number_db[9]->outertext;
//    var_dump($str_next);die;
