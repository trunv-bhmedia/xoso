...
function get_register_receive_ms($area)
{
	$db = new MyDBO();
	$today = date('Y-m-d');
	$sql = "select * from xs_iphone_dv_register_receive_ms where area_receive='".$area."' and status=1 and nexttime!='$today' order by time desc limit 25";
	//echo $sql;
	$rows = $db->get_rows($sql);	
	return $rows;
}
...
if($area !=0){
//_push("cea11a0b3692bc34a04c883b2be647466095edfed992902dacbe75694483b6ca", $message); echo "xong"; die;
	...
	//echo $message;
	//_push("cea11a0b3692bc34a04c883b2be647466095edfed992902dacbe75694483b6ca", $message); die("SSSSSSSSS");
	//die;
	//var_dump($xml->status);die;
	...
	if($xml->status =='quayxong'){		
		foreach($arrayReg as $k=>$item2)
		{
			...
			//$device_token = "cea11a0b3692bc34a04c883b2be647466095edfed992902dacbe75694483b6ca";
			if($arrayDev[0]->android == 0){
				//for ($k=0; $k<$countk; $k++){ //lay ra 25 devices de gui push
					if(_push($device_token, $message))
					{
...
function refresh($param, $time = 4000 )
	{
		...
		<html>
			...
			<center>
				...
				<form name="form_refresh_getcontent" action="push.php" method="GET">
					<?php
