...
<div class="select-provice t-cen rate-lo clearfix">
    <form id="form_search" method="post" action="">
        ...
        <div class="rows seque clearfix">
            ...
            <span class="span-input space"><input type="text" name="amplitude" class="txt-input txt-liver" value="<?php echo $amplitude ?>" /></span>
            <a class="read-more" href="javascript:;" onclick="submitForm('thong-ke-lo-gan');"><span>Xem kết quả</span></a>
        </div>
    </form>
</div>
<div class="page-title t-cen">
    <strong><h1>Thống kê gan cực đại Xổ số <?php echo $lname ?></h1></strong>
</div>
<?php
if (count($items)) {
    ...
    foreach ($items as $key => $value) {
        ?>
        <div class="title-tk"><strong>Thống kê Gan cực đại dãy số <?php echo $key ?> xổ số <?php echo $lname ?> từ ngày: <?php echo $fromdate ?> - <?php echo $todate ?></strong></div>
        <?php
        if (!$value) {
