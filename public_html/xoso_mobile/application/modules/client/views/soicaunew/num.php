...
<h1 style="position: absolute; text-indent: -99999px">Tìm cầu cho cặp số</h1>
<div class="page-title-xs"><strong>Tìm cầu cho cặp số</strong></div>
<div class="box-result">
    <form name=f action='<?php echo $uri_root ?>soi-cau.html' method="get" style='padding:10px;text-align:center;font-size:12px'>
        <input type=hidden name=submit value='1' />
        <input type=hidden name=setmode value='num' />
        <b>Tìm tất cả các cầu cho cặp số:&nbsp;</b>
        <input type=text size=10 name='timcau' value='<?php echo $timcau ?>' style='font-size:13px; font-family:arial,sans-serif; border:#8d4343 1px solid; font-weight:bold; color:#bc1c18; padding:0 0 0 2px; height:23px' title='Có thể nhập từng số cách nhau bằng dấu phẩy hoặc cả 1 dãy số. Ví dụ 00-99 sẽ soi tất cả từ 00 đến 99' />
        <div style="padding:5px 0 0">
        &nbsp;với độ dài thấp nhất:&nbsp;
        ...
        <div id=opts style='margin:0 auto;background:#FFF5E8; border:#F3DCB1 1px solid; padding:5px'>
...
<!--        <div class="ablue"><a href='javascript:;' onclick='dlg("currkq",530,500); return false'>>> Xem kết quả ngày <?php echo str_replace('-', '/', $ngay) ?></a></div>-->
    <?php } ?>

    <div style="text-align:center" id=showcauarea></div>
    <div class="vitri_block" style="padding-left:10px">
        <?php
        if ($timcau != '') {
            $mang_so = explode(',', $timcau);
            ...
            foreach ($mang_so as $so) {
                ...
                if (!isset($ds_cau[$data_so])) {
                    echo "<div style='color:#CC0000; font-weight:bold'>Không tìm thấy cầu nào >= " . $limit . " ngày cho cặp số " . $data_so . "</div>";
                } else {
                    echo "<div style='margin:10px 0; font-size:15px; color:#008000'>Các vị trí cầu tìm thấy cho cặp số <b style='font-size:17px'>" . $data_so . "</b>:</div>";
                    $dem = 0;
                    $arr_vitri = array();
                    foreach ($ds_cau[$data_so] as $vitri => $cau) {
                        ...
                        $dem++;
                        $url = 'showcau&amp;ngay=' . $ngay . '&amp;limit=' . $cau . '&amp;exactlimit=0&amp;lon=' . $lon . '&amp;nhay=' . $nhay . '&amp;db=' . $db . '&amp;vt=' . $vitri;
                        echo "<div class='ablue' style='margin:5px 0'><a target='_blank' href='?" . $url . "#" . $url . "'>" . $dem . ". Độ dài: <b>" . $cau . "</b> ngày, tại vị trí <b>" . $vitri . "</b></a></div>";
                    }
                    ?>
                    <div style='display:none' id="matrix<?php echo $so . str_replace('-', '', $ngay) ?>" title='MA TRẬN CẦU' class='matrancau_dlg'>
                    ...
                    <div style='margin:15px 0'><a class=a_button_light href='javascript:;' onclick='matrancaudlg("matrix<?php echo $so . str_replace('-', '', $ngay) ?>"); return false' title='Xem phân bố vị trí của cầu trên bảng kết quả'>Ma trận cầu</a></div>
                    <?php
                }
            }
        }
