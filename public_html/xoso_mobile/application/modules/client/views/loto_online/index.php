<h1 style="position: absolute; text-indent: -99999px">Tài khoản Loto</h1>
<div class="page-title-xs"><strong>Tài khoản Loto</strong></div>
...
<div class="contentcontainer">
    <div id='loonlinenote' style='color:#414042; margin:5px 0;text-align:justify'>Lotto Online tại Xoso.com là trò chơi miễn phí và <b>không cần nạp tiền</b>, nghĩa là bạn không cần phải có tiền trước trong tài khoản mà có thể ghi ngay tối đa 2000 điểm cho mỗi cặp số. Số tiền trong tài khoản của bạn không có giá trị rút thành tiền thật.</div>
    <script type='text/javascript'>closable('#loonlinenote')</script>
    <?php if (!isset($_SESSION["user"])) { ?>
        <div class=msg>Hãy <a href='<?php echo $uri_root ?>dang-nhap.html'>đăng nhập</a> để sử dụng chức năng ghi và quản lý lôtô. <a href='<?php echo $uri_root ?>dang-ky.html'>Đăng ký tại đây</a> nếu bạn chưa có tài khoản.</div>
        <br/>
    <?php } ?>
    <style type="text/css">
        ...
        #lichsu{overflow:auto}
        .lichsu{margin:0; border-collapse:collapse}
    ...
    <form id=betform name=betform action='' onsubmit='return betformsubmit()' style='font-size:13px; font-weight:bold; color:#353535; padding:3px; margin-bottom:5px'>
        <input type=hidden size=8 id=ngay name=ngay style='font-size:12px; color:#8B2001' value='<?php echo date('d/m/Y', strtotime($date)) ?>' /></span>
    ...
    <?php if (isset($_SESSION["user"])) { ?>
        ...
        <div id='betarea' style='border:#cdcdcd 1px solid; display:none'>
            <div style='background:#f2f2f2; padding:5px'>
                <a name="betplace"></a> 
                <div style='font-weight:bold'>Ghi lô ngày <b id='ngaydisplay'><?php echo date('d/m/Y', strtotime($date)) ?></b></div>
    ...
    <?php if (isset($_SESSION["user"])) { ?>
        <div style='margin-top:10px; border:#cdcdcd 1px solid;'>
