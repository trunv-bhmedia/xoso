...
if ($alias == $url_mientrung) {
    $l_area = 'MIỀN TRUNG';
    $url = 'so-dau-duoi/mien-trung';
} else {
    $l_area = 'MIỀN NAM';
    $url = 'so-dau-duoi/mien-nam';
}
if (isset($date) && $date != '')
    ...
    foreach ($items as $xoso) {
        ...
        foreach ($xoso as $value) {
            if ($value->name == 'Tp. Hồ Chí Minh')
                $value->name = 'TP. HCM';
            $title .= '<td class="kq"><span class="name">' . $value->name . '</span><span class="xanh">' . $value->a8 . '</span> - <span class="do">' . substr($value->a0, -2, 2) . '</span></td>';
        }
        ?>
