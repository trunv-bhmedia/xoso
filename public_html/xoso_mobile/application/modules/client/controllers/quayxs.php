...
class quayxs extends Client {
    ...
    public function index() {
        $this->data['_meta'] = $this->meta_model->show_title('default');
        $this->data['_meta']['title'] = 'Quay số cầu may';
        $this->data['tmpl'] = 'quayxs/somm';
        $this->load->view('layout/content', $this->data);
    }

    public function quaythu() {
        $this->data['lid'] = (isset($_POST['lid']) ? trim($_POST['lid']) : 1);
        ...
        $this->data['alias'] = '';
        $this->data['area'] = '';
        foreach ($this->data['location_today'] as $items) {
        ...
        $this->data['_meta'] = $this->meta_model->show_title('default');
        $this->data['_meta']['title'] = 'Quay số chuẩn';
        $this->data['tmpl'] = 'quayxs/quaythu';
        $this->load->view('layout/content', $this->data);
    }

    public function quaynhanh() {
        $t = (isset($_GET['t']) ? $_GET['t'] : 1);

        $this->data['_meta'] = $this->meta_model->show_title('quayxs');
        if ($t == 1)
            $this->data['tmpl'] = 'quayxs/index';
        else
            $this->data['tmpl'] = 'quayxs/mn';
        $this->load->view('layout/content', $this->data);
    }
    ...
    public function loadKq() {
        ...
        ?>
        <ul class="gr-yellow">
            <h2 class="pad10-5 txt-center">Kết quả xổ số <a href="<?php echo $alias ?>.html"><?php echo $name ?></a> ngày <?php echo $date ?></h2>
        </ul>
        <?php
        if ($area == 'MB') {
            ?>
            <ul class="list-kqmb">
                ...
                <li class="bg_yellow pad5 clearfix">
                    ...
                    <div class="aprize clred s24"><span><strong class="imgloadig" id="g27"></strong></span></div>
                </li>
            </ul>
            <?php
        } else {
            ...
            <ul class="list-kqmb city-mn">
                <li class="bg_yellow pad5 clearfix">
                    ...
                    <div class="aprize clred s24"><span><strong class="imgloadig" id="g1"></strong></span></div>
                </li>
                <li class="pad5 clearfix">
                    <label class="fl"><strong>Giải bảy</strong></label>
                ...
                <li class="bg_yellow pad5 clearfix">
                    <label class="special fl"><strong>ĐB</strong></label>
                    <div class="aprize clred s24">
