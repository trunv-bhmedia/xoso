...
function get_cat($datedove) {
    $db = new MyDBO();
    $sql = "SELECT * FROM `vietlott_data` WHERE type = 3 AND date like '$datedove%' LIMIT 1";
        ...
        for ($k = 0; $k < $countInput; $k++) {
            ...
            for ($i = 0; $i < 6; $i++) {
                if (in_array($itemDay[$i], $boso)) {
                    ...
                    $countTG++;
                } elseif($itemDay[$i] == $itemPowKQ) {
                    $strout .= '<span class="active power">' . $itemDay[$i] . '</span>';
                    $trungPow = 1;
                } else {
                    $strout .= '<span>' . $itemDay[$i] . '</span>';
                }
            }
            if ($countTG == 5 && $trungPow == 1) {
                $arrDayTrung[][10] = $strout;
            } else if ($countTG > 2) {
                ...
                <div id="kqxs_matran">
                                ...
                                <tbody>
                                    ...
                                    for ($i = 0; $i < $countItem; $i++) {
                                        ...
                                        <tr class="web_bg_Trang">
                                            ...
                                            <?php
                                            $arrDayTrung[$i];
                                            foreach ($arrDayTrung[$i] as $gaiTrung => $itemdtvalue) {
                                                ?>
                    ...
                    <div class="boxseo bogoc2">
                        <a href="#" class="send-sms" data-telno="9911" data-message="MEGA">
                            <h3 style="font-size: 14px;color: green; text-transform: uppercase; font-weight: bold"><center>Chúc mừng bạn đã trúng giải</center></h3>
                        </a>
                    </div>        
                </div>
            </div>					
